...
public class LibrarianDashboardController {
    ...
    @GetMapping("/profile")
    @Operation(summary = "Get my profile", description = "Get current librarian profile")
    public ResponseEntity<LibrarianSummaryResponse> getMyProfile(@AuthenticationPrincipal User user) {
        com.education.sms.entity.Librarian librarian = librarianRepository.findByUserId(user.getId())
                .orElseThrow(() -> new com.education.sms.exception.ResourceNotFoundException("Librarian not found"));

        return ResponseEntity.ok(new LibrarianSummaryResponse(
                librarian.getId(),
                librarian.getId(),
                librarian.getFirstName(),
                librarian.getLastName(),
                librarian.getFirstName() + " " + librarian.getLastName(),
                librarian.getUser().getEmail(),
                librarian.getEmployeeId(),
                librarian.getPhone(),
                librarian.getUser().getProfileImageUrl()));
    }
