...
public class FacultyDashboardController {
        ...
        @GetMapping("/profile")
        @Operation(summary = "Get my profile", description = "Get current faculty profile")
        public ResponseEntity<FacultySummaryResponse> getMyProfile(
                        @Parameter(hidden = true) @AuthenticationPrincipal User user) {
                Faculty faculty = facultyRepository.findByUserId(user.getId())
                                .orElseThrow(() -> new ResourceNotFoundException("Faculty not found"));

                return ResponseEntity.ok(new FacultySummaryResponse(
                                faculty.getId(),
                                ...
                                faculty.getPhone(),
                                faculty.getUser().getProfileImageUrl()));
        }
        ...
        @Operation(summary = "Get my subjects", description = "Get subjects assigned to the current faculty")
        public ResponseEntity<?> getMySubjects(@Parameter(hidden = true) @AuthenticationPrincipal User user) {
                Faculty faculty = facultyRepository.findByUserId(user.getId())
                                .orElseThrow(() -> new ResourceNotFoundException("Faculty not found"));

                List<Subject> subjects = subjectRepository.findByFacultyId(faculty.getId());

                var subjectList = subjects.stream()
                                .map(s -> new SubjectSummary(
                                                s.getId(),
                                                s.getName(),
                                                s.getCode(),
                                                s.getClassEntity() != null ? s.getClassEntity().getId() : null,
                                                s.getClassEntity() != null
                                                                ? s.getClassEntity().getGradeLevel() + " - "
